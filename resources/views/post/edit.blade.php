        ...
        <form class="form-horizontal" role="form" action="{{ route('post') }}" method="POST">
            ...
            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-9">
                    <input type="text" id="title" name="title" placeholder="Title"
                           class="col-xs-10 col-sm-5" value="{{ $oPost->title }}" required>
                    @if ($errors->has('title'))
            ...
            <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-9">
                    <input type="text" id="description" name="description" placeholder="Description"
                           class="col-sm-12 col-xs-12" value="{{ $oPost->description }}" required>
            ...
            <div class="form-group {{ $errors->has('seq') ? 'has-error' : '' }}">
                ...
                <div class="col-xs-12 col-sm-9">
                    <select id="recipient" name="recipient[]" class="multiselect" multiple="" disabled >
                        @foreach ($aRecipient as $oRecipient)
                            <option value="{{ $oRecipient->id }}" {{ $oPost->recipient()->where('user_id', '=', $oRecipient->id)->get()->isEmpty() === false ? 'selected' : ''}}>{{ $oRecipient->name }}</option>
                        @endforeach
                    </select>
                    <a id="changeRecipient" href="#">Change</a>
                    @if ($errors->has('seq'))
                        <span class="help-inline col-xs-12 col-sm-12">
            ...
            <div id="fuelux-wizard-container">
                <div>
                    <ul class="steps">
                        @if (count($oPost->recipient) > 1)
                            @for($x = 0; $x < count($oPost->recipient); $x++)
                                <li data-step="{{ $x + 1 }}">
                                    <span class="step"> {{ $x + 1 }}</span>
                                    <span class="title"> {{  $oPost->recipient[$x]->user->name }}</span>
                                </li>
                            @endfor
                        @endif
                    </ul>
...
@section('page-level-script')
    ...
    <script>
        ...
        $('#changeRecipient').on('click', function () {
            $('.multiselect')
